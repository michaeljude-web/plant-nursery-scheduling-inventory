...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_variety_report'])) {
    ...
    echo "<script>alert('Damage report submitted and quantity updated successfully.'); window.location.href='plot.php';</script>";
    exit();
}
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container mt-4">
        <h2 class="text-center mb-4">Create Planting Plots</h2>

        <div class="row mb-4">
            <div class="col-md-6 mb-2">
                <input type="text" id="searchBar" class="form-control" placeholder="Search seed name...">
            </div>
    ...
    <div class="col-md-4 seedling-item mb-4" data-name="<?= strtolower($row['seed_name']) ?>" data-category="<?= $row['category_name'] ?>">
        <div class="card h-100">
            <h5><?= $row['seed_name'] ?> - <?= $row['variety_name'] ?></h5>
            <p><strong>Category:</strong> <?= $row['category_name'] ?></p>
            <p><strong>Total Planted (Current Quantity):</strong> <?= $row['total_planted'] ?></p>

            <form method="POST">
    <input type="hidden" name="seedling_variety_id" value="<?= $row['seedling_variety_id'] ?>">

    <label class="form-label fw-bold">Insert Plots (max 50):</label>

    <div class="input-group mb-3 align-items-center">
        <input type="number" name="plot_count" min="1" max="50" required 
        ...
        <button type="button" class="btn btn-primary btn-sm me-2" 
                data-bs-toggle="modal" data-bs-target="#transferModal<?= $row['seedling_variety_id'] ?>">
            <i class="fas fa-boxes me-1"></i> For Sale

        <button type="button" class="btn btn-danger btn-sm" 
                data-bs-toggle="modal" data-bs-target="#reportModal<?= $row['seedling_variety_id'] ?>">
            ...
            <div class="modal fade" id="transferModal<?= $row['seedling_variety_id'] ?>" tabindex="-1" aria-labelledby="transferModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <form method="POST" class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="transferModalLabel">Transfer Seedlings to Inventory - <?= $row['seed_name'] ?> (<?= $row['variety_name'] ?>)</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <input type="hidden" name="seedling_variety_id_transfer" value="<?= $row['seedling_variety_id'] ?>">
                                <div class="mb-3">
                                    <label>Transfer Quantity</label>
                                    <input type="number" name="transfer_quantity" min="1" max="<?= $row['total_planted'] ?>" class="form-control" required>
                                    <small class="form-text text-muted">Maximum available quantity: <?= $row['total_planted'] ?></small>
                                </div>
                            </div>
...
<div class="modal fade" id="reportModal<?= $row['seedling_variety_id'] ?>" tabindex="-1">
    <div class="modal-dialog">
        <form method="POST" enctype="multipart/form-data" class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Report Damage - <?= $row['seed_name'] ?> (<?= $row['variety_name'] ?>)</h5>
            ...
            <div class="modal-footer">
                <button type="submit" name="submit_variety_report" class="btn btn-primary">Submit Report</button>
            </div>
        </form>
    </div>
</div>
